...
class WaterQualityDataset(Dataset):
    ...
    def preprocess_data(self):
        ...
        targets = self.target_dataset['Dissolved Oxygen'].values
...
class LSTMModel(nn.Module):
    def __init__(self, hidden_size = 64, num_layers = 4, dropout = 0.2):
...
def objective(trial):
    ...
    # train
    last_val_loss = train_model(model, train_loader, val_loader, lr = learning_rate, epochs = epochs)
...
def train_and_test():
    ...
    feature_dataset = read_data(PATH_KNN_FEATURES)
    target_dataset = read_data(PATH_KNN)

    # feature_dataset = read_data(PATH_MINI_KNN_FEATURES)
    # target_dataset = read_data(PATH_KNN)
